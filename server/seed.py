...
with app.app_context():
    ...
    for x in range(60):
        donation = Donation(amount=fake.random_int(min=500, max=10000), donor_id=fake.random_int(min=348, max=378), anonymous=random.choice(approval_status), organization_id=fake.random_int(min=133, max=153))
    ...
    for x in range(70):
        beneficiaries = Beneficiary(name=fake.name(), recieved_amount=fake.random_int(min=300, max=1000), image_url="https://pbs.twimg.com/media/FpmD9sMXEAA4Mb7.jpg", organization_id=fake.random_int(min=133, max=143) )
